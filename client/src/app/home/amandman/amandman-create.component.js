...
(function () {
  ...
  AmandmanCreateController.$inject = ['$scope', '$stateParams', 'GenericResource', 'exception', '$state', 'AmandmanSpecification'];

  function AmandmanCreateController($scope, $stateParams, GenericResource, exception, $state, AmandmanSpecification) {
    var vm = this;
    ...
    function getAktReferences(akt) {
      ...
      for (var i = 0; i < clanovi.length; i++) {
        akt_references.clanovi.push({
          value: clanovi[i].id,
          caption: clanovi[i].naziv
        });
      }
      ...
      for (var i = 0; i < tacke.length; i++) {
        for (var j = 0; j < tacke[i].content.length; j++) {
          if ((tacke[i].content[j]).hasOwnProperty("content")) { //ako nema content onda je referenca
            ...
            akt_references.podtacke.push({
              value: podtacka.id,
              caption: podtacka.naziv
            });
          }
        }
      }
